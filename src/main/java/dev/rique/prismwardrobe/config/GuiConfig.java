...
public final class GuiConfig {
    ...
    public static GuiConfig from(FileConfiguration config) {
        ...
        builder.slotDisplayIndices = sanitizeSlots(slotDisplayIndices, COLUMNS_PER_PAGE);

        // Armor layout rows
        ConfigurationSection armorLayout = config.getConfigurationSection("layout.armor-layout");
        if (armorLayout != null) {
            builder.helmetRow = armorLayout.getInt("helmet-row", 0);
            builder.chestplateRow = armorLayout.getInt("chestplate-row", 1);
            builder.leggingsRow = armorLayout.getInt("leggings-row", 2);
            builder.bootsRow = armorLayout.getInt("boots-row", 3);
        } else {
            builder.helmetRow = 0;
        ...
        if (equipButtons != null) {
            builder.equipButtonsRow = equipButtons.getInt("row", 4);
            ...
            builder.equipButtonSlots = sanitizeSlots(equipSlots, maxIndexExclusive);
        } else {
        ...
        builder.controlFillerSlots = sanitizeSlots(controlSlots, maxIndexExclusive);
