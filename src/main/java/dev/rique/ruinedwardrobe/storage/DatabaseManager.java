    ...
    public void initialize(File dataFolder) {
        ...
        if (storage.type() == DatabaseType.SQLITE) {
            ...
            if (parent != null && !parent.exists() && !parent.mkdirs()) {
                throw new DatabaseException("Failed to create SQLite database directory: " + parent.getAbsolutePath());
            }
            hikariConfig.setJdbcUrl("jdbc:sqlite:" + databaseFile.getAbsolutePath());
            hikariConfig.setDriverClassName("org.sqlite.JDBC");
            poolSize = Math.min(4, storage.maxPoolSize());
            ...
            poolSize = storage.maxPoolSize();
            hikariConfig.setMaximumPoolSize(poolSize);
            hikariConfig.setMinimumIdle(Math.min(storage.maxPoolSize(), storage.minIdle()));
        }
        ...
        hikariConfig.setPoolName("RuinedWardrobePool");
        ...
        int workerThreads = Math.min(executionSettings.workerThreads(), poolSize);
        executor = new ThreadPoolExecutor(
                ...
                runnable -> {
                    Thread thread = new Thread(runnable, "RuinedWardrobe-DB");
