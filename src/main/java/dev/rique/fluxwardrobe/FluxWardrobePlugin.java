package dev.rique.fluxwardrobe;

import dev.rique.fluxwardrobe.api.WardrobeApi;
import dev.rique.fluxwardrobe.command.WardrobeCommand;
import dev.rique.fluxwardrobe.config.ConfigManager;
import dev.rique.fluxwardrobe.config.DatabaseType;
import dev.rique.fluxwardrobe.config.PluginConfig;
import dev.rique.fluxwardrobe.core.HealthMonitor;
import dev.rique.fluxwardrobe.core.WardrobeArmorBindingService;
import dev.rique.fluxwardrobe.core.WardrobeArmorSyncService;
import dev.rique.fluxwardrobe.core.PermissionTierResolver;
import dev.rique.fluxwardrobe.core.RestrictionServiceImpl;
import dev.rique.fluxwardrobe.core.SlotLimitServiceImpl;
import dev.rique.fluxwardrobe.core.VersionSyncService;
import dev.rique.fluxwardrobe.core.WardrobeApiImpl;
import dev.rique.fluxwardrobe.core.WardrobeServiceImpl;
import dev.rique.fluxwardrobe.gui.WardrobeGuiController;
import dev.rique.fluxwardrobe.gui.WardrobeGuiListener;
import dev.rique.fluxwardrobe.integration.combat.CombatProviderFactory;
import dev.rique.fluxwardrobe.integration.combat.CombatTagProvider;
import dev.rique.fluxwardrobe.integration.placeholder.FluxWardrobeExpansion;
import dev.rique.fluxwardrobe.integration.vault.VaultHook;
import dev.rique.fluxwardrobe.lang.LanguageRegistry;
import dev.rique.fluxwardrobe.lang.MessageService;
import dev.rique.fluxwardrobe.listener.PlayerSessionListener;
import dev.rique.fluxwardrobe.listener.WardrobeArmorProtectionListener;
import dev.rique.fluxwardrobe.scheduler.SchedulerAdapter;
import dev.rique.fluxwardrobe.scheduler.SchedulerFactory;
import dev.rique.fluxwardrobe.storage.DatabaseManager;
import dev.rique.fluxwardrobe.storage.MigrationService;
import dev.rique.fluxwardrobe.storage.SqlWardrobeRepository;
...
public final class FluxWardrobePlugin extends JavaPlugin {
    ...
    private FluxWardrobeExpansion placeholderExpansion;
    ...
    public void onEnable() {
        try {
            bootstrap();
            getLogger().info("FluxWardrobe enabled successfully.");
        } catch (Exception ex) {
            getLogger().severe("Failed to enable FluxWardrobe: " + ex.getMessage());
            ex.printStackTrace();
    ...
    private void initializeIntegrations(PluginConfig pluginConfig) {
        ...
        if (pluginConfig.integrationSettings().placeholderApiEnabled() && Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
            placeholderExpansion = new FluxWardrobeExpansion(this, wardrobeService, slotLimitService);
