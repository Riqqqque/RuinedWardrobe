...
public final class SqlWardrobeRepository implements WardrobeRepository {
    ...
    private static final String PLAYERS_TABLE = "fw_players";
    private static final String SETS_TABLE = "fw_sets";
    private static final String META_TABLE = "fw_meta";
    private static final String VERSION_INDEX = "idx_fw_players_version";
    ...
    private static final SchemaLayout CURRENT_SCHEMA = new SchemaLayout(
            ...
            "legs_data",
            "boots_data");
    private static final SchemaLayout LEGACY_SCHEMA = new SchemaLayout(
            "pw_players",
            "pw_sets",
            "pw_meta",
            "helmet_json",
    ...
    private void createPlayersTable(Connection connection) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
                CREATE TABLE IF NOT EXISTS fw_players (
    ...
    private void createSetsTable(Connection connection) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
                CREATE TABLE IF NOT EXISTS fw_sets (
    ...
    private void createVersionIndex(Connection connection) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
                CREATE INDEX IF NOT EXISTS idx_fw_players_version
                ON fw_players(version)
                """)) {
            statement.execute();
        }
    }
    ...
    private LegacyImportResult importLegacySchemaIfNeeded(Connection connection) throws SQLException, IOException {
        if (!legacySchemaExists(connection)) {
            return LegacyImportResult.none();
        }
        if (currentSchemaHasData(connection)) {
            return new LegacyImportResult("Detected legacy wardrobe tables, but FluxWardrobe tables already contain data. Skipping automatic legacy import.");
        }

        MigrationSnapshot preparedLegacySnapshot = prepareLegacySnapshot(readSnapshot(connection, LEGACY_SCHEMA));
        ...
        return new LegacyImportResult(
                "Imported legacy wardrobe data into FluxWardrobe. Players: " + preparedLegacySnapshot.players().size()
    ...
    private boolean legacySchemaExists(Connection connection) throws SQLException {
        return tableExists(connection, LEGACY_SCHEMA.playersTable())
                || tableExists(connection, LEGACY_SCHEMA.setsTable())
                || tableExists(connection, LEGACY_SCHEMA.metaTable());
    }
    ...
    private void dropLegacySchema(Connection connection) throws SQLException {
        execute(connection, "DROP TABLE IF EXISTS " + LEGACY_SCHEMA.metaTable());
        execute(connection, "DROP TABLE IF EXISTS " + LEGACY_SCHEMA.setsTable());
        execute(connection, "DROP TABLE IF EXISTS " + LEGACY_SCHEMA.playersTable());
    }
        ...
        execute(connection, """
                ...
                WHERE selected_slot > 0
                  AND NOT EXISTS (
                      SELECT 1
                      FROM fw_sets
                      WHERE fw_sets.player_uuid = fw_players.player_uuid
                        AND fw_sets.slot_index = fw_players.selected_slot
                  )
                """);
    }
